...
func NewClientOperationDuration() ClientOperationDuration {
     labels := []string{AttrOperationName("").Key(),AttrProviderName("").Key(),error.AttrType("").Key(),AttrRequestModel("").Key(),server.AttrPort("").Key(),AttrResponseModel("").Key(),server.AttrAddress("").Key(),}
     return ClientOperationDuration{ 
        HistogramVec: prometheus.NewHistogramVec(prometheus.HistogramOpts{
            Name: "gen_ai_client_operation_duration",
            Help: "GenAI operation duration.",
     }, labels)}
}

func (m ClientOperationDuration) With(operationName AttrOperationName,providerName AttrProviderName,extras ...interface {ErrorType() error.AttrType;GenAiRequestModel() AttrRequestModel;ServerPort() server.AttrPort;GenAiResponseModel() AttrResponseModel;ServerAddress() server.AttrAddress;}) prometheus.Observer { 
        if extras == nil { extras = append(extras, m.extra) }
        extra := extras[0]
    
    return m.HistogramVec.WithLabelValues(operationName.Value(),providerName.Value(),extra.ErrorType().Value(),extra.GenAiRequestModel().Value(),extra.ServerPort().Value(),extra.GenAiResponseModel().Value(),extra.ServerAddress().Value(),)
}
...
func (a ClientOperationDuration) WithRequestModel (attr interface { GenAiRequestModel() AttrRequestModel } ) ClientOperationDuration {
...
func (a ClientOperationDuration) WithResponseModel (attr interface { GenAiResponseModel() AttrResponseModel } ) ClientOperationDuration {
...
func (a ClientOperationDurationExtra) GenAiRequestModel() AttrRequestModel {return a.AttrRequestModel}
func (a ClientOperationDurationExtra) ServerPort() server.AttrPort {return a.AttrServerPort}
func (a ClientOperationDurationExtra) GenAiResponseModel() AttrResponseModel {return a.AttrResponseModel}
