...
func main() {
  ...
  builder := dashboard.NewDashboardBuilder("signalr").
    Uid("signalr-dashboard").
		...
		timeseries.NewPanelBuilder().
			...
			WithTarget(
				prometheus.NewDataqueryBuilder().
					Expr("rate(signalr.server.active_connections[$__rate_interval])").
					Range().
	...
	WithPanel(
		timeseries.NewPanelBuilder().
			...
			WithTarget(
				prometheus.NewDataqueryBuilder().
					Expr("rate(signalr.server.connection.duration[$__rate_interval])").
					Range().
					Format(prometheus.PromQueryFormatTimeSeries).
					LegendFormat("__auto"),
			),
