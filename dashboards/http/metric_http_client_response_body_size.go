...
type ClientResponseBodySize struct {
     *prometheus.HistogramVec
     extra ClientResponseBodySizeExtra
}



func NewClientResponseBodySize() ClientResponseBodySize {
     labels := []string{AttrRequestMethod("").Key(),server.AttrAddress("").Key(),server.AttrPort("").Key(),error.AttrType("").Key(),AttrResponseStatusCode("").Key(),network.AttrProtocolName("").Key(),url.AttrTemplate("").Key(),network.AttrProtocolVersion("").Key(),url.AttrScheme("").Key(),}
...
func (m ClientResponseBodySize) With(requestMethod AttrRequestMethod,address server.AttrAddress,port server.AttrPort,extras ...interface {ErrorType() error.AttrType;HttpResponseStatusCode() AttrResponseStatusCode;NetworkProtocolName() network.AttrProtocolName;UrlTemplate() url.AttrTemplate;NetworkProtocolVersion() network.AttrProtocolVersion;UrlScheme() url.AttrScheme;}) prometheus.Observer { 
        if extras == nil { extras = append(extras, m.extra) }
        extra := extras[0]
    
    return m.HistogramVec.WithLabelValues(requestMethod.Value(),address.Value(),port.Value(),extra.ErrorType().Value(),extra.HttpResponseStatusCode().Value(),extra.NetworkProtocolName().Value(),extra.UrlTemplate().Value(),extra.NetworkProtocolVersion().Value(),extra.UrlScheme().Value(),)
}
...
func (a ClientResponseBodySize) WithNetworkProtocolName (attr interface { NetworkProtocolName() network.AttrProtocolName } ) ClientResponseBodySize {
...
func (a ClientResponseBodySize) WithNetworkProtocolVersion (attr interface { NetworkProtocolVersion() network.AttrProtocolVersion } ) ClientResponseBodySize {
...
func (a ClientResponseBodySizeExtra) ErrorType() error.AttrType {return a.AttrErrorType}
func (a ClientResponseBodySizeExtra) HttpResponseStatusCode() AttrResponseStatusCode {return a.AttrResponseStatusCode}
func (a ClientResponseBodySizeExtra) NetworkProtocolName() network.AttrProtocolName {return a.AttrNetworkProtocolName}
func (a ClientResponseBodySizeExtra) UrlTemplate() url.AttrTemplate {return a.AttrUrlTemplate}
func (a ClientResponseBodySizeExtra) NetworkProtocolVersion() network.AttrProtocolVersion {return a.AttrNetworkProtocolVersion}
func (a ClientResponseBodySizeExtra) UrlScheme() url.AttrScheme {return a.AttrUrlScheme}
...
State {
    ...
    auto_escape: None,
    ctx: {
        "AttrExtra": "ClientResponseBodySizeExtra",
        "Instr": "Histogram",
        "InstrMap": {
            "counter": "Counter",
            "gauge": "Gauge",
        ...
        "Name": "client.response.body.size",
