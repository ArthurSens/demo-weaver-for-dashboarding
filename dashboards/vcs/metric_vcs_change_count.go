...
func NewChangeCount() ChangeCount {
     labels := []string{AttrChangeState("").Key(),AttrRepositoryUrlFull("").Key(),AttrOwnerName("").Key(),AttrRepositoryName("").Key(),AttrProviderName("").Key(),}
     return ChangeCount{ 
        GaugeVec: prometheus.NewGaugeVec(prometheus.GaugeOpts{
            Name: "vcs_change_count",
            Help: "The number of changes (pull requests/merge requests/changelists) in a repository, categorized by their state (e.g. open or merged).",
     }, labels)}
}

func (m ChangeCount) With(changeState AttrChangeState,repositoryUrlFull AttrRepositoryUrlFull,extras ...interface {VcsOwnerName() AttrOwnerName;VcsRepositoryName() AttrRepositoryName;VcsProviderName() AttrProviderName;}) prometheus.Gauge { 
        if extras == nil { extras = append(extras, m.extra) }
        extra := extras[0]
    
    return m.GaugeVec.WithLabelValues(changeState.Value(),repositoryUrlFull.Value(),extra.VcsOwnerName().Value(),extra.VcsRepositoryName().Value(),extra.VcsProviderName().Value(),)
}

// Deprecated: Use [ChangeCount.With] instead
func (m ChangeCount) WithLabelValues(lvs ...string) prometheus.Gauge {
    return m.GaugeVec.WithLabelValues(lvs...)
}

func (a ChangeCount) WithOwnerName (attr interface { VcsOwnerName() AttrOwnerName } ) ChangeCount {
    a.extra.AttrOwnerName = attr.VcsOwnerName()
    return a
}
func (a ChangeCount) WithRepositoryName (attr interface { VcsRepositoryName() AttrRepositoryName } ) ChangeCount {
    a.extra.AttrRepositoryName = attr.VcsRepositoryName()
    return a
}
func (a ChangeCount) WithProviderName (attr interface { VcsProviderName() AttrProviderName } ) ChangeCount {
...
func (a ChangeCountExtra) VcsOwnerName() AttrOwnerName {return a.AttrOwnerName}
func (a ChangeCountExtra) VcsRepositoryName() AttrRepositoryName {return a.AttrRepositoryName}
func (a ChangeCountExtra) VcsProviderName() AttrProviderName {return a.AttrProviderName}
