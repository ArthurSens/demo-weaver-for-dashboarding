...
func NewRejectedConnections() RejectedConnections {
     labels := []string{network.AttrTransport("").Key(),network.AttrType("").Key(),server.AttrAddress("").Key(),server.AttrPort("").Key(),}
     return RejectedConnections{ 
        CounterVec: prometheus.NewCounterVec(prometheus.CounterOpts{
            Name: "kestrel_rejected_connections",
            Help: "Number of connections rejected by the server.",
     }, labels)}
}

func (m RejectedConnections) With(extras ...interface {NetworkTransport() network.AttrTransport;NetworkType() network.AttrType;ServerAddress() server.AttrAddress;ServerPort() server.AttrPort;}) prometheus.Counter { 
        if extras == nil { extras = append(extras, m.extra) }
        extra := extras[0]
    
    return m.CounterVec.WithLabelValues(extra.NetworkTransport().Value(),extra.NetworkType().Value(),extra.ServerAddress().Value(),extra.ServerPort().Value(),)
}

// Deprecated: Use [RejectedConnections.With] instead
func (m RejectedConnections) WithLabelValues(lvs ...string) prometheus.Counter {
    return m.CounterVec.WithLabelValues(lvs...)
}

func (a RejectedConnections) WithNetworkTransport (attr interface { NetworkTransport() network.AttrTransport } ) RejectedConnections {
    a.extra.AttrNetworkTransport = attr.NetworkTransport()
    return a
}
func (a RejectedConnections) WithNetworkType (attr interface { NetworkType() network.AttrType } ) RejectedConnections {
    a.extra.AttrNetworkType = attr.NetworkType()
    return a
}
func (a RejectedConnections) WithServerAddress (attr interface { ServerAddress() server.AttrAddress } ) RejectedConnections {
    a.extra.AttrServerAddress = attr.ServerAddress()
    return a
}
func (a RejectedConnections) WithServerPort (attr interface { ServerPort() server.AttrPort } ) RejectedConnections {
...
func (a RejectedConnectionsExtra) NetworkTransport() network.AttrTransport {return a.AttrNetworkTransport}
func (a RejectedConnectionsExtra) NetworkType() network.AttrType {return a.AttrNetworkType}
func (a RejectedConnectionsExtra) ServerAddress() server.AttrAddress {return a.AttrServerAddress}
func (a RejectedConnectionsExtra) ServerPort() server.AttrPort {return a.AttrServerPort}
